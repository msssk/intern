define([
	...
	var htmlReporter = {
		...
		'/suite/start': function (suite) {
			...
			// intern-geezer does not calculate these values
			// Duplicating this for intern does no harm
			suite.numTests = 0;
		...
		'/suite/end': function (suite) {
			...
			if (indentLevel) {
				// If we're in a nested suite, append test counts to parent suite
				suite.parent.numTests += suite.numTests;
				suite.parent.numFailedTests += suite.numFailedTests;
			}
			else {
				// Otherwise update the global tracking variables
